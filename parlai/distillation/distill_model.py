...
"""
Model distillation script for for blenderbot models.
...
def setup_args(parser=None) -> ParlaiParser:
    ...
    train.add_argument('--config-path',
                        type=str,
                        help='config path for teacher and student model'
    )
    train.add_argument(
        '--student-model-file',
        type=str,
...
class TrainLoop:
    ...
    def __init__(self, opt):
        ...
        if (
            ...
            and opt.get('student_model_file')
            and os.path.isfile(opt['student_model_file'] + '.checkpoint')
        ):
            opt['init_model_student'] = opt['student_model_file'] + '.checkpoint'
            trainstats_suffix = '.checkpoint.trainstats'
        # Possibly build a dictionary (not all models do this).
        if not (opt.get('dict_file') or opt.get('student_model_file')):
            raise RuntimeError(
                'WARNING: For train_model, please specify either a '
                'student_model_file or dict_file.'
            )
        if 'dict_file' in opt:
        ...
        if opt.get('student_model_file') and os.path.isfile(
            opt['student_model_file'] + trainstats_suffix
        ):
            # looks like we were preempted. make sure we load up our total
            # training stats, etc
            with open(opt['student_model_file'] + trainstats_suffix) as ts:
                obj = json.load(ts)
                ...
                if 'best_valid' in obj:
                    ...
                    # old method
                    if opt.get('student_model_file') and os.path.isfile(
                        opt['student_model_file'] + '.best_valid'
                    ):
                        with open(opt['student_model_file'] + ".best_valid", 'r') as f:
                            x = f.readline()
    ...
    def save_model(self, suffix=None):
        ...
        if not self.opt.get('student_model_file'):
            # nothing to save to, just exit
            return

        fn = self.opt['student_model_file']
        if suffix:
            fn += suffix
        while True:
    ...
    def validate(self):
        ...
        # saving
        if (
            opt.get('student_model_file')
            and opt.get('save_after_valid')
            and is_primary_worker()
        ):
            logging.info(f"saving model checkpoint: {opt['student_model_file']}.checkpoint")
    ...
    def _run_eval(self, valid_worlds, opt, datatype, max_exs=-1, write_log=False):
        ...
        # write to file
        if write_log and opt.get('student_model_file') and is_primary_worker():
            # Write out metrics
            f = open(opt['student_model_file'] + '.' + datatype, 'a+')
            f.write(f'{metrics}\n')
            f.close()
        ...
        with world:
            while True:
                ...
                if (
                    ...
                    logging.info(
                        f"saving model checkpoint: {opt['student_model_file']}.checkpoint"
                    )
                    if opt['tensorboard_log'] and is_primary_worker():
                        self.tb_logger.flush()
                    self.save_model('.checkpoint')
